...
async def summarize(db: AsyncSession, input_data_id: int, text: str) -> list[ReportItem]:
...
async def generate_final_report(included_items: list[ReportItem], title: str) -> str:
    ...
    try:
        return await claude_client.chat(prompt, system=system)
    except Exception as e:
